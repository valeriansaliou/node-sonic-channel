...
/**
 * SonicChannelGeneric.prototype._truncateCommandContent
 * @protected
 * @param  {string} content
 * @return {string} Truncated content
 */
SonicChannelGeneric.prototype._truncateCommandContent = function(content) {
  // Content should be truncated? (prevents buffer overflows)
  if (this.__environment._contentTruncateLimit  &&
        content.length > this.__environment._contentTruncateLimit) {
    return content.substring(0, this.__environment._contentTruncateLimit);
  }
...
SonicChannelGeneric.prototype.__handleDataLine_started = function(
  ...
  if (typeof environmentParsed.buffer !== "undefined") {
    // Truncate to 75% of the allowed in-buffer characters, so that we leave \
    //   a bit of character space for other parts of the command. Then, divide \
    //   by 4 as an UTF-8 character contains a maximum of 4 bytes.
    environmentParsed._contentTruncateLimit = (
      Math.floor((environmentParsed.buffer * 0.75) / 4)
    );
  }
