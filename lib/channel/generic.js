...
var SonicChannelGeneric = function(mode, options) {
  ...
  var emitQueueMaxSizeDefault    = 500;
    ...
    try {
      ...
      client.on("close", function() {
        if (isConnected === true) {
          ...
          // Failure (closed)
          self.__triggerConnectHandler(
            "disconnected", (self.__lastError || null)
          );
  ...
  return this._wrapDone(function(done) {
    if (this.__client !== null) {
      this.__execute("ping", [done]);
        ...
        null, new Error("Channel is disconnected, cannot send ping")
      );
    }
  });
};
...
SonicChannelGeneric.prototype._emit = function(command, done, client) {
  ...
  if (this.__pendingEmit.callback === undefined) {
    ...
    // Emit queue is full? (destroy client and force a reconnect)
    if (this.__emitQueue.length > this.__options.emitQueueMaxSize) {
      (client || this.__client).destroy(
        new Error(
...
SonicChannelGeneric.prototype.__setupPingInterval = function(client, do_setup) {
  ...
  if (do_setup === true) {
    ...
    this.__pingInterval = setInterval(function() {
      ...
      var pingDeadTimeout = setTimeout(function() {
        client.destroy(
          new Error(
