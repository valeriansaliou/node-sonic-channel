...
SonicChannelSearch.prototype.query = function(
  ...
  return this.__executeOrDefer("query", [
...
/**
 * SonicChannelSearch.prototype.__operation_$query
 * @private
 * @param  {string|object} cacheBucketID
 * @param  {function}      done
 * @return {undefined}
 */
SonicChannelSearch.prototype.__operation_$query = function(
  collection, bucket, terms, limit, offset, done
) {
  // Generate query values
  var queryValues = [collection, bucket];

  // TODO: shell-escape (new lines + quotes + etc.)
  // TODO: do this in-house via fn provided by generic (no dep)
  queryValues.push("\"" + terms + "\"");
